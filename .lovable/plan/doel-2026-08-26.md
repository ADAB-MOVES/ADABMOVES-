...
1. In `src/routes/index.tsx` de `head()` aanpassen:
  ...
  - `name="description"` en `og:description` → `"ADAB MOVES is een multisportaanbieder voor kinderen en jongeren in Amsterdam en omstreken. We bieden wekelijkse sportlessen en workshops voor scholen en community ."`
...
2. In `src/routes/__root.tsx` de fallback meta gelijk trekken, zodat ook andere routes consistente defaults hebben:
  ...
  - fallback `description`, `og:description`, `twitter:description` → `"ADAB MOVES is een multisportaanbieder voor kinderen en jongeren in Amsterdam en omstreken. We geven wekelijkse sportlessenen workshops voor scholen en community"`
