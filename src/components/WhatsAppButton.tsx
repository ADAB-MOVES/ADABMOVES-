import { PHONE_RAW } from "@/lib/whatsapp";

export function WhatsAppButton() {
  const href = `https://api.whatsapp.com/send?phone=${PHONE_RAW}&text=${encodeURIComponent(
    "Hallo, ik wil graag meer weten over ADAB MOVES.",
  )}`;
  return (
    <a
      href={href}
      ...
      className="fixed bottom-5 right-5 z-50 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_rgba(37,211,102,0.45)] transition-transform hover:scale-110"
    >
      <span aria-hidden className="pointer-events-none absolute inset-0 rounded-full animate-pulse-ring" />
      <svg
